...
코드는 이미 구현되어 있습니다. Firebase 초기화 및 푸시 알림 서비스가 자동으로 설정됩니다.

### 5.2 Push Notification Service 생성

#### 파일: `lib/core/services/internal/push_notification_service.dart`

푸시 알림 서비스가 이미 구현되어 있으며, 채널별 푸시 알림을 지원합니다.

### 5.3 로그인 시 토큰 저장

로그인 성공 시 FCM 토큰을 자동으로 저장하도록 `AuthViewModel`에 구현되어 있습니다.
...
`PushNotificationService`에서 정적 채널은 앱 초기화 시 자동으로 생성되며, 동적 채널은 `createChatRoomChannel()` 메서드를 통해 생성할 수 있습니다.
...
final response = await SupabaseService.client.functions.invoke(
  ...
  body: {
    ...
    'channelId': 'chat_room_$roomId', // 동적 채널 사용
    'data': {
      'type': 'chat',
      'roomId': roomId,
      'messageId': 'msg_456',
